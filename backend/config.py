...
class Settings:
    ...
    ollama_model: str = os.getenv("MINERVA_OLLAMA_MODEL", "qwen3.5:9b")
    ollama_temperature: float = float(os.getenv("MINERVA_OLLAMA_TEMPERATURE", "0.3"))
    ollama_max_tokens: int = int(os.getenv("MINERVA_OLLAMA_MAX_TOKENS", "1024"))
    ollama_think: bool = os.getenv("MINERVA_OLLAMA_THINK", "false").lower() == "true"
    agent_timeout_seconds: int = int(os.getenv("MINERVA_AGENT_TIMEOUT", "300"))
