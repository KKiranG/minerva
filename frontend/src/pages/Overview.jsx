...
const lastAnalysisLabel = (value) => {
  ...
  return {
    primary: asDate(value),
    secondary: dayDelta <= 0 ? 'Today' : `${dayDelta} day${dayDelta === 1 ? '' : 's'} ago`,
    tone: freshness === 'Fresh' ? 'positive' : freshness === 'Recent' ? 'info' : 'warning',
    freshness,
  };
};
    ...
    <div className="space-y-6">
      ...
      >
        <div className="space-y-3">
          {rows.map((stock) => {
            const analysis = lastAnalysisLabel(stock.last_analysis_date);
            return (
              <a
                key={stock.ticker}
                ...
                <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
                  ...
                  <div className="grid gap-3 sm:grid-cols-2 xl:min-w-[560px] xl:grid-cols-4">
                    ...
                    <div className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3">
                      ...
                      <div className="mt-2 text-lg font-semibold text-white">{analysis.primary}</div>
                      <div className="text-sm text-slate-300">{analysis.secondary}</div>
                    </div>
                  </div>
                </div>
              </a>
