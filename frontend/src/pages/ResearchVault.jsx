...
import { Badge, Button, EmptyState, LoadingState, PageSection, Select, TextInput } from '../components/Common';
...
export default function ResearchVault({ api, stocks = [] }) {
  const [tab, setTab] = useLocalStorage('minerva_vault_tab', 'notes');
  const [ticker, setTicker] = useLocalStorage('minerva_vault_ticker', 'ALL');
  const [category, setCategory] = useLocalStorage('minerva_vault_category', 'ALL');
  const [query, setQuery] = useLocalStorage('minerva_vault_query', '');
  const debouncedQuery = useDebounce(query, 150);
  const [dateFrom, setDateFrom] = useLocalStorage('minerva_vault_from', '');
  const [dateTo, setDateTo] = useLocalStorage('minerva_vault_to', '');
  ...
  return (
          ...
          {tab === 'notes' ? (
            <div className="grid gap-2 md:grid-cols-5">
              ...
              <div className="flex gap-2">
                <TextInput placeholder="Search notes" value={query} className="flex-1" onChange={(event) => setQuery(event.target.value)} />
                <Button type="button" onClick={() => api.downloadResearchCsv({ ticker: ticker === 'ALL' ? undefined : ticker })}>
                  Export CSV
                </Button>
              </div>
            </div>
    ...
    >
      {tab === 'templates' ? (
        ...
        <LoadingState type="list" items={6} />
